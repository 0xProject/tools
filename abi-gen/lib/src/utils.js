...
exports.utils = {
    solTypeToAssertion(solName, solType) {
    ...
    solTypeToTsType(paramKind, backend, solType, components) {
        const trailingArrayRegex = /\[\d*\]$/;
        if (solType.match(trailingArrayRegex)) {
            const arrayItemSolType = solType.replace(trailingArrayRegex, '');
            const arrayItemTsType = exports.utils.solTypeToTsType(paramKind, backend, arrayItemSolType, components);
            const arrayTsType = exports.utils.isUnionType(arrayItemTsType) || exports.utils.isObjectType(arrayItemTsType)
            ...
            if (solType.match(TUPLE_TYPE_REGEX)) {
                const componentsType = _.map(components, component => {
                    const componentValueType = exports.utils.solTypeToTsType(paramKind, backend, component.type, component.components);
    ...
    solTypeToPyType(dataItem) {
        const solType = dataItem.type;
        const trailingArrayRegex = /\[\d*\]$/;
        if (solType.match(trailingArrayRegex)) {
            const arrayItemPyType = exports.utils.solTypeToPyType(Object.assign(Object.assign({}, dataItem), { type: dataItem.type.replace(trailingArrayRegex, '') }));
            ...
            const TUPLE_TYPE_REGEX = '^tuple$';
            if (solType.match(TUPLE_TYPE_REGEX)) {
                return exports.utils.makePythonTupleName(dataItem);
            }
            throw new Error(`Unknown Solidity type found: ${solType}`);
        }
    },
    ...
    getPartialNameFromFileName(filename) {
        const name = path.parse(filename).name;
        return name;
    },
    getNamedContent(filename) {
        const name = exports.utils.getPartialNameFromFileName(filename);
        try {
    ...
    writeOutputFile(filePath, renderedTsCode) {
        fs.writeFileSync(filePath, renderedTsCode);
    },
    isOutputFileUpToDate(outputFile, sourceFiles) {
    ...
    makePythonTupleName(tuple) {
        if (tuple.internalType !== undefined) {
            ...
            return `Tuple0x${crypto_1.createHash('MD5')
     ...
     */
    makePythonTupleClassBody(tupleComponents) {
        let toReturn = '';
        for (const tupleComponent of tupleComponents) {
            toReturn = `${toReturn}\n\n    ${tupleComponent.name}: ${exports.utils.solTypeToPyType(tupleComponent)}`;
        }
        toReturn = `${toReturn}`;
        return toReturn;
    },
     ...
     */
    wrapPythonDocstringRole(docstring, indent) {
    ...
    extractTuples(parameter, tupleBodies, // output
    tupleDependencies) {
        if (parameter.type === 'tuple' || parameter.type === 'tuple[]') {
            ...
            const pythonTupleName = exports.utils.makePythonTupleName(tupleDataItem);
            ...
            for (const component of tupleDataItem.components) {
                if (component.type === 'tuple' || component.type === 'tuple[]') {
                    tupleDependencies.push([
                        exports.utils.makePythonTupleName(component),
                        pythonTupleName,
                    ]);
