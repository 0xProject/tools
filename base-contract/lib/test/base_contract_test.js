...
describe('BaseContract', () => {
    describe('strictArgumentEncodingCheck', () => {
        ...
        it('throws for integer overflows', () => {
            expect(() => src_1.BaseContract.strictArgumentEncodingCheck([{ name: 'amount', type: 'uint8' }], ['256'])).to.throw();
        });
        it('throws for fixed byte array overflows', () => {
            expect(() => src_1.BaseContract.strictArgumentEncodingCheck([{ name: 'hash', type: 'bytes8' }], ['0x001122334455667788'])).to.throw();
        });
    });
});
